<?php
	...
	if(!empty($member)){
		foreach($member as $index => $value){
			if($value['officer'] == 1){
				echo '<a href="'.base_url().'tour_controller/load_id/user/'.$value['id'].'" class="officer" title="officer">'.$value['username'].'</a>';
			}else{
				echo '<a href="'.base_url().'tour_controller/load_id/user/'.$value['id'].'">'.$value['username'].'</a>';
			}
